...
setwd("C:\\Users\\User\\Documents\\distribution metro\\Nov 24")
...
### Set core controls
apollo_control = list(
  modelName       = "MNL_Access_BigModel_Correction",
  modelDescr      = "Simple MNLtotal model on mode choice egress data - for bus, rick, walk",
  indivID         = "ID", 
  outputDirectory = "Jan_09_access_without_gp"
)
...
database = read.csv("ENCODED_ACCESS1+2.csv",header=TRUE)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate") {
  ...
  V[["rick"]] = asc_rick + b_cost_rick * rick_cost +  #b_tt_rick * rick_tt #  b_dist_walk * walk_dist + b_dist_rick * rick_dist
                #b_11am_rick*(surv_t==2) + b_1pm_rick*(surv_t==3) + b_5pm_rick*(surv_t==5) +
                b_3pm_rick*(surv_t==4) +  #b_7pm_rick*(surv_t==6) + 
                b_st_moti_rick*(ac_st==16) + b_st_du_rick*(ac_st==14) + b_st_shbg_rick*(ac_st==13) + #b_st_karbz_rick*(ac_st==12) + 
                b_st_farm_ricK*(ac_st==11) +  b_st_un_rick*(ac_st==1) +#b_st_agar_rick*(ac_st==9) + b_st_mir10_rick*(ac_st==6) + b_st_mir11_rick*(ac_st==5) + b_st_pllb_rick*(ac_st==4) + 
                #b_tp_business_rick*(trip_purp==1) + 
                b_tp_edu_rick*(trip_purp==2) + #b_tp_medi_rick*(trip_purp==3) + 
                b_tp_job_rick*(trip_purp==4) + #b_tp_rec_rick*(trip_purp==6) + 
                b_tp_rtrn_rick*(trip_purp==7) + #b_tp_shop_rick*(trip_purp==8)+ 
                ...
                #b_TF1_rick*(trip_freq==1) + 
                b_TF2_rick*(trip_freq==2) + #b_TF3_rick*(trip_freq==3) + 
                b_TF4_rick*(trip_freq==4) + b_TF5_rick*(trip_freq==5) + #b_TF6_rick*(trip_freq==6) +
                b_other_rick*b5 + #b_bag_rick*b1 + b_lugg_rick*b3 + b_shpbag_rick*b4 + b_obag_rick*b2 
                b_comp_rick*ac_comp + 
                ...
                #b_inc2_rick*(month_inc==20) + b_inc3_rick*(month_inc==30) + 
                b_inc4_rick*(month_inc==60) + b_inc5_rick*(month_inc==80) + #b_inc6_walk*(month_inc==150)
                #b_own_car_rick*veh_own_car + 
                b_own_bike_rick*veh_own_bike  #b_own_cyc_rick*veh_own_cyc +
                #b_mrt_rick*mrt_pass
  
  V[["bus"]]  = asc_bus + b_tt_bus * bus_tt + b_cost_bus * bus_cost + # b_dist_bus * bus_dist + b_dist_bus * bus_dist+
                #b_11am_bus*(surv_t==2) + b_1pm_bus*(surv_t==3) + b_5pm_bus*(surv_t==5) +
                b_3pm_bus*(surv_t==4) + b_7pm_bus*(surv_t==6) +
               b_st_du_bus*(ac_st==14) + b_st_shbg_bus*(ac_st==13) + #b_st_karbz_bus*(ac_st==12) + 
                b_st_farm_bus*(ac_st==11) + b_st_agar_bus*(ac_st==9) + b_st_mir10_bus*(ac_st==6) + #b_st_moti_bus*(ac_st==16) +b_st_mir11_bus*(ac_st==5) + b_st_pllb_bus*(ac_st==4) + b_st_un_bus*(ac_st==1) +
                #b_tp_edu_bus*(trip_purp==2) + #b_tp_business_bus*(trip_purp==1) + #b_tp_job_bus*(trip_purp==4)  b_tp_medi_bus*(trip_purp==3) + #b_tp_rtrn_bus*(trip_purp==7) 
               #b_tp_rec_bus*(trip_purp==6) + #b_tp_shop_bus*(trip_purp==8) +
                ...
                b_obag_bus*b2 + #b_shpbag_bus*b4 + b_bag_bus*b1 +b_other_bus*b5 + #b_lugg_bus*b3 
                b_comp_bus*ac_comp +  
                #b_age1_bus*(age==20) + 
                b_age2_bus*(age==30) + #b_age3_bus*(age==40) +# b_age4_bus*(age==60)+
                b_fmale_bus*(gen==1) +
                b_stud_bus*(occ==1) +  #b_gov_bus*(occ==3) + #b_hwife_bus*(occ==2) +
                b_pvt_bus*(occ==4) + #b_bzman_bus*(occ==5) + #b_rtd_bus*(occ==6)+ #b_unemp_bus*(occ==7) +
